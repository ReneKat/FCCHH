...
for file in `ls ${reads_dir}/*.f*q.gz`; do
  ...
  counts=$(gunzip -c $file | wc -l)
  seqs=$(expr $counts / 4)
  echo $file,$seqs >> ${reads_dir}/00_raw_reads_counts.csv
done
...
source activate /home/renekat/anaconda3/envs/pp


echo starting quality control on ${project} raw reads in ${reads_dir}
...
while read -r sample; do
  ...
  echo Assessing intial read quality on ${sample}
  ...
  bbduk.sh in=./01_HQ_Reads/atrimmed.fq.gz out=./01_HQ_Reads/qtrimmed.fq.gz qtrim=rl trimq=10 ftm=5 trimpolygright=0 minlen=70 ordered maxns=0 maq=8 entropy=.95 ow=t >> ./01_HQ_Reads/logs/${sample}.log 2>&1

  #ftl=16 to remove the 16bp of barcode left on the 5' read. Removes Nextera 5' noise
  #bbduk.sh in=./01_HQ_Reads/atrimmed.fq.gz out=./01_HQ_Reads/qtrimmed.fq.gz ftl=16 qtrim=rl trimq=10 ftm=5 trimpolygright=0 minlen=70 ordered maxns=0 maq=8 entropy=.95 ow=t >> ./01_HQ_Reads/logs/${sample}.log 2>&1
...
for sample in `cat samples.txt`; do
  ...
  source activate /home/renekat/anaconda3/envs/FCH_assembly

  read -r -p "Which HQ reads dataset do you want to use to assemble $sample? HQ HQ_ecc HQ_merged :  " HQ_Reads || exit 100

  echo ${HQ_Reads} reads will be used to assemble $sample.
...
## Do not make a directory for megahit, it will make it itself and will fail if the output directory already exists, as to not overwrite results. ##
  ...
  if [[ "${HQ_Reads}" == HQ ]]
  then
    echo start metaspades on ${sample}
    ...
    echo start megahit on sample ${sample}
#    megahit --12 ./01_HQ_Reads/${sample}_HQ.fq.gz --presets meta-large -m ${RAM} -t ${CPU} -o /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample} >> ./02_Assembly/megahit-large/logs/${sample}.log 2>&1
    megahit --12 ./01_HQ_Reads/${sample}_HQ.fq.gz --k-min 21 --k-max 91 --k-step 10 -m ${RAM} -t ${CPU} -o /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample} >> ./02_Assembly/megahit-large/logs/${sample}.log 2>&1
    cp  /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}/final.contigs.fa ./02_Assembly/quast/${sample}/megahit.fasta
    mkdir -p ./02_Assembly/megahit-large/${sample}
    mv /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample} ./02_Assembly/megahit-large/${sample}
    #IDBA can only take fasta files as input; therefore, fastq.gz files must be reformated to fasta files
    reformat.sh in=./01_HQ_Reads/${sample}_HQ.fq.gz out=./01_HQ_Reads/${sample}_HQ.fa -Xmx${RAM}G >> ./02_Assembly/idba_ud/logs/${sample}.log 2>&1
    # #Assemble reads per site using idba_ud
    echo start idba_ud on sample ${sample}
    ...
    # --- Evaluation ---
      echo metagenome_id,reads,assembler,n_contigs,contig_bp,gap_pct,ctg_L50,ctg_max,gene_counts,mapped_pct,avg_cov >> ./02_Assembly/stats/${sample}_assembly_stats.csv
      echo Evaluating ${sample} assemblies with AssemblyStats
      statswrapper.sh minscaf=1000 ./02_Assembly/quast/${sample}/*.fasta format=3 -Xmx${RAM}G out=./02_Assembly/stats/${sample}
      stats.sh minscaf=1000 ./02_Assembly/metaspades/${sample}/scaffolds.fasta format=3 -Xmx${RAM}G out=./02_Assembly/metaspades/${sample}/stats.txt
      spades_stats=$(cat ./02_Assembly/metaspades/${sample}/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
      stats.sh minscaf=1000 ./02_Assembly/megahit-large/${sample}/final.contigs.fa format=3 -Xmx${RAM}G out=./02_Assembly/megahit-large/${sample}/stats.txt
      megahit_stats=$(cat ./02_Assembly/megahit-large/${sample}/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
      stats.sh minscaf=1000 ./02_Assembly/idba_ud/${sample}/scaffold.fa format=3 -Xmx${RAM}G out=./02_Assembly/idba_ud/${sample}/stats.txt
      ...
      spades_match=$(grep Percent\ mapped: ./02_Assembly/metaspades/${sample}/map.log | cut -f2 -d$'\t')
      spades_cov=$(grep Average\ coverage: ./02_Assembly/metaspades/${sample}/map.log | cut -f2 -d$'\t')
      ...
      megahit_match=$(grep Percent\ mapped: ./02_Assembly/megahit-large/${sample}/map.log | cut -f2 -d$'\t')
      megahit_cov=$(grep Average\ coverage: ./02_Assembly/megahit-large/${sample}/map.log | cut -f2 -d$'\t')
      ...
      idba_match=$(grep Percent\ mapped: ./02_Assembly/idba_ud/${sample}/map.log | cut -f2 -d$'\t')
      idba_cov=$(grep Average\ coverage: ./02_Assembly/idba_ud/${sample}/map.log | cut -f2 -d$'\t')

      echo Creating assembly comparision for ${sample}
      echo ${sample},${HQ_Reads},metaspades,${spades_stats},${spades_counts},${spades_match},${spades_cov} >> ./02_Assembly/stats/${sample}_assembly_stats.csv
      echo ${sample},${HQ_Reads},megahit,${megahit_stats},${megahit_counts},${megahit_match},${megahit_cov} >> ./02_Assembly/stats/${sample}_assembly_stats.csv
      echo ${sample},${HQ_Reads},idba_ud,${idba_stats},${idba_counts},${idba_match},${idba_cov} >> ./02_Assembly/stats/${sample}_assembly_stats.csv
  ...
  #  megahit --12 ./01_HQ_Reads/error_corrected/${sample}_HQ_ecc.fq.gz --presets meta-large -m ${RAM} -t ${CPU} -o ./02_Assembly/megahit-large/${sample} >> ./02_Assembly/megahit-large/logs/${sample}.log 2>&1
    megahit --12 ./01_HQ_Reads/error_corrected/${sample}_HQ_ecc.fq.gz --k-min 21 --k-max 91 --k-step 10 -m ${RAM} -t ${CPU} -o /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}_ecc >> ./02_Assembly/megahit-large/logs/${sample}.log 2>&1
    cp /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}_ecc/final.contigs.fa ./02_Assembly/quast/${sample}/megahit_ecc.fasta
    mkdir -p ./02_Assembly/megahit-large/${sample}_ecc
    mv /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}_ecc/ ./02_Assembly/megahit-large/${sample}_ecc/

    #IDBA can only take fasta files as input; therefore, fastq.gz files must be reformated to fasta files
    reformat.sh in=./01_HQ_Reads/error_corrected/${sample}_HQ_ecc.fq.gz out=./01_HQ_Reads/error_corrected/${sample}_HQ_ecc.fa -Xmx${RAM}G >> ./02_Assembly/idba_ud/logs/${sample}.log 2>&1
    # #Assemble reads per site using idba_ud
    echo start idba_ud on sample ${sample}
    ...
    # --- Evaluation ---
      echo metagenome_id,reads,assembler,n_contigs,contig_bp,gap_pct,ctg_L50,ctg_max,gene_counts,mapped_pct,avg_cov >> ./02_Assembly/stats/${sample}_assembly_stats.csv
      echo Evaluating ${sample} assemblies with AssemblyStats
      stats.sh minscaf=1000 ./02_Assembly/metaspades/${sample}_ecc/scaffolds.fasta format=3 -Xmx${RAM}G out=./02_Assembly/metaspades/${sample}_ecc/stats.txt
      spades_stats=$(cat ./02_Assembly/metaspades/${sample}_ecc/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
      stats.sh minscaf=1000 ./02_Assembly/megahit-large/${sample}_ecc/final.contigs.fa format=3 -Xmx${RAM}G out=./02_Assembly/megahit-large/${sample}_ecc/stats.txt
      megahit_stats=$(cat ./02_Assembly/megahit-large/${sample}_ecc/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
      stats.sh minscaf=1000 ./02_Assembly/idba_ud/${sample}_ecc/scaffold.fa format=3 -Xmx${RAM}G out=./02_Assembly/idba_ud/${sample}_ecc/stats.txt
      ...
      spades_match=$(grep Percent\ mapped: ./02_Assembly/metaspades/${sample}_ecc/map.log | cut -f2 -d$'\t')
      spades_cov=$(grep Average\ coverage: ./02_Assembly/metaspades/${sample}_ecc/map.log | cut -f2 -d$'\t')
      ...
      megahit_match=$(grep Percent\ mapped: ./02_Assembly/megahit-large/${sample}_ecc/map.log | cut -f2 -d$'\t')
      megahit_cov=$(grep Average\ coverage: ./02_Assembly/megahit-large/${sample}_ecc/map.log | cut -f2 -d$'\t')
      ...
      idba_match=$(grep Percent\ mapped: ./02_Assembly/idba_ud/${sample}_ecc/map.log | cut -f2 -d$'\t')
      idba_cov=$(grep Average\ coverage: ./02_Assembly/idba_ud/${sample}_ecc/map.log | cut -f2 -d$'\t')
...
#    megahit -r ./01_HQ_Reads/merged_reads/${sample}_HQ_merged.fq.gz --12 ./01_HQ_Reads/merged_reads/${sample}_HQ_unmerged.fq.gz --presets meta-large -m ${RAM} -t ${CPU} -o ./02_Assembly/megahit-large/${sample} >> ./02_Assembly/megahit-large/logs/${sample}.log 2>&1
    megahit -r ./01_HQ_Reads/merged_reads/${sample}_HQ_merged.fq.gz --12 ./01_HQ_Reads/merged_reads/${sample}_HQ_unmerged.fq.gz --k-min 21 --k-max 91 --k-step 10 -m ${RAM} -t ${CPU} -o /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}_merged >> ./02_Assembly/megahit-large/logs/${sample}.log 2>&1
    cp /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}_merged/final.contigs.fa ./02_Assembly/quast/${sample}/megahit_merged.fasta
    mkdir -p ./02_Assembly/megahit-large/${sample}_merged/
    mv /mnt/c/Users/Student/CAMI_LOW/02_Assembly/megahit-large/${sample}_merged/ ./02_Assembly/megahit-large/${sample}_merged/

    echo idba_ud does not assemble merged reads and will not be used for comparison.
    echo metagenome_id,reads,assembler,n_contigs,contig_bp,gap_pct,ctg_L50,ctg_max,gene_counts,mapped_pct,avg_cov >> ./02_Assembly/stats/${sample}_assembly_stats.csv
    echo Evaluating ${sample} assemblies with AssemblyStats
    stats.sh minscaf=1000 ./02_Assembly/metaspades/${sample}_merged/scaffolds.fasta format=3 -Xmx${RAM}G out=./02_Assembly/metaspades/${sample}_merged/stats.txt
    spades_stats=$(cat ./02_Assembly/metaspades/${sample}_merged/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
    stats.sh minscaf=1000 ./02_Assembly/megahit-large/${sample}_merged/final.contigs.fa format=3 -Xmx${RAM}G out=./02_Assembly/megahit-large/${sample}_merged/stats.txt
    ...
    spades_match=$(grep Percent\ mapped: ./02_Assembly/metaspades/${sample}_merged/map.log | cut -f2 -d$'\t')
    spades_cov=$(grep Average\ coverage: ./02_Assembly/metaspades/${sample}_merged/map.log | cut -f2 -d$'\t')
    ...
    megahit_match=$(grep Percent\ mapped: ./02_Assembly/megahit-large/${sample}_merged/map.log | cut -f2 -d$'\t')
    megahit_cov=$(grep Average\ coverage: ./02_Assembly/megahit-large/${sample}_merged/map.log | cut -f2 -d$'\t')
...
# --- Evaluation ---
  ...
  echo Evaluating ${sample} assemblies with AssemblyStats
  statswrapper.sh minscaf=1000 ./02_Assembly/quast/${sample}/*.fasta format=3 -Xmx${RAM}G out=./02_Assembly/stats/${sample}
  metaquast.py -f -m 1000 -o ./02_Assembly/quast/${sample} ./02_Assembly/quast/${sample}/*.fasta >> ./02_Assembly/quast/${sample}.log 2>&1
  ...
  echo Reformating ${sample} fasta file and removing short contigs
  anvi-script-reformat-fasta --simplify-names --prefix ${prefix} -r ./03_Anvio/${sample}/${sample}_rename_key.txt -l 1000 -o ./03_Anvio/${sample}/${sample}_renamed_c1k.fa ${best_assembly} >> ./03_Anvio/logs/${sample}.log 2>&1
  ...
  mkdir -p ./04_Annotations/antismash/${sample}_c50k
  echo Running antismash for secondary metabolite annotation
  ~/bin/run_antismash \
  ./03_Anvio/${sample}/${sample}_renamed_c1k.fa \
  ./04_Annotations/antismash/${sample}_c50k \
  --hmmdetection-strictness relaxed \
  --logfile ${sample}_c50k.log \
  --cb-general \
  --cb-knownclusters \
  --cb-subclusters \
  ...
  -c ${CPU}
    ...
    cd ./04_Annotations/antismash/${sample}_c50k
    # echo ${sample}
      for genbank_file in $(ls c*.gbk); do
        cluster=$(echo ${genbank_file} | cut -f1 -d '_')
        echo copying and renaming ${genbank_file} to ${cluster}_${sample}.gbk
        cp ${genbank_file} ${project_dir}/04_Annotations/bigscape/${cluster}_${sample}.gbk
      done
    cd ${project_dir}
  done
...
#
# #IDBA can only take fasta files as input; therefore, fastq.gz files must be reformated to fasta files
# reformat.sh in=./01_HQ_Reads/${project}_HQ.fq.gz out=./01_HQ_Reads/${project}_HQ.fa -Xmx${RAM}G >> ./02_Co-Assembly/logs/${project}.log 2>&1
#
# # #Assemble reads per site using idba_ud
...
#
# echo metagenome_id,assembler,n_contigs,contig_bp,gap_pct,ctg_L50,ctg_max,gene_counts,mapped_pct,avg_cov >> ./02_Co-Assembly/stats/${project}_co-assembly_stats.csv
# echo Evaluating ${project} assemblies with AssemblyStats
# # statswrapper.sh ./02_Co-Assembly/quast/metaspades.fasta ./02_Co-Assembly/quast/megahit.fasta ./02_Co-Assembly/quast/idba.fasta format=3 -Xmx${RAM}G out=./02_Co-Assembly/stats
# stats.sh ./02_Co-Assembly/metaspades/metaspades.fasta format=3 -Xmx${RAM}G out=./02_Co-Assembly/metaspades/stats.txt
# spades_stats=$(cat ./02_Co-Assembly/metaspades/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
# stats.sh ./02_Co-Assembly/megahit-large/megahit.fasta format=3 -Xmx${RAM}G out=./02_Co-Assembly/megahit-large/stats.txt
# megahit_stats=$(cat ./02_Co-Assembly/megahit-large/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
# stats.sh ./02_Co-Assembly/idba_ud/idba.fasta format=3 -Xmx${RAM}G out=./02_Co-Assembly/idba_ud/stats.txt
# idba_stats=$(cat ./02_Co-Assembly/idba_ud/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
# # Evaluate stats for merged assemblies
# stats.sh ./02_Co-Assembly/metaspades/merged_reads/metaspades_merged.fasta format=3 -Xmx${RAM}G out=./02_Co-Assembly/metaspades/merged_reads/stats.txt
# spades_merged_stats=$(cat ./02_Co-Assembly/metaspades/merged_reads/stats.txt | cut -f2,4,5,9,15 -d$'\t' | sed '2q;d' | sed 's/\t/,/g')
# stats.sh ./02_Co-Assembly/megahit-large/merged_reads/megahit_lg_merged.fasta format=3 -Xmx${RAM}G out=./02_Co-Assembly/megahit-large/merged_reads/stats.txt
...
# spades_match=$(grep Percent\ mapped: ./02_Co-Assembly/metaspades/map.log | cut -f2 -d$'\t')
# spades_cov=$(grep Average\ coverage: ./02_Co-Assembly/metaspades/map.log | cut -f2 -d$'\t')
#
...
# megahit_match=$(grep Percent\ mapped: ./02_Co-Assembly/megahit-large/map.log | cut -f2 -d$'\t')
# megahit_cov=$(grep Average\ coverage: ./02_Co-Assembly/megahit-large/map.log | cut -f2 -d$'\t')
#
...
# idba_match=$(grep Percent\ mapped: ./02_Co-Assembly/idba_ud/map.log | cut -f2 -d$'\t')
# idba_cov=$(grep Average\ coverage: ./02_Co-Assembly/idba_ud/map.log | cut -f2 -d$'\t')
#
...
# spades_merged_match=$(grep Percent\ mapped: ./02_Co-Assembly/metaspades/merged_reads/map.log | cut -f2 -d$'\t')
# spades__merged_cov=$(grep Average\ coverage: ./02_Co-Assembly/metaspades/merged_reads/map.log | cut -f2 -d$'\t')
#
...
# megahit_merged_match=$(grep Percent\ mapped: ./02_Co-Assembly/megahit-large/merged_reads/map.log | cut -f2 -d$'\t')
# megahit_merged_cov=$(grep Average\ coverage: ./02_Co-Assembly/megahit-large/merged_reads/map.log | cut -f2 -d$'\t')
#
...
# anvi-script-reformat-fasta --simplify-names --prefix ${project} -r ./02_Co-Assembly/${project}_rename_key.txt  -l 2000 -o ./03_anvio/${project}_reformated_assembly_c1k.fa ${best_coassembly} >> ./03_anvio/logs/${project}.log 2>&1
...
#   bbmap.sh in=./01_HQ_Reads/${sample}_HQ_reads.fq.gz out=./03_anvio/${sample}.sam bamscript=bs.sh outm=./03_anvio/${sample}/${sample}_mapped.fq.gz outu=./03_anvio/${sample}/${sample}_unmapped.fq.gz ihist=./03_anvio/${sample}/${sample}_insertsize.txt interleaved=t pigz=t threads=12 -Xmx${RAM}G >> ./03_anvio/logs/${project}.log 2>&1
